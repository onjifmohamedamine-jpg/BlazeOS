This Repo is for BLAZEOS
This distro is a Study project based on $\color{emerald}{\text{Linux Mint Xena}}$ that is also based on $\color{orange}{\text{Ubuntu noble}}$ 
This project is open-source if you want to Edit it
Thanks Canonical for Making ubuntu Open-source

Download link:
https://mega.nz/fm/ujxThRAL

Please Enjoy your experience, enable Dark node for the best experience possible!
